...
from evia_journal.contract import read_csv, read_json, write_json
...
class ContractTests(unittest.TestCase):
    def test_schema_is_identical_to_public_browser_contract(self):
        installed = json.loads(
            files("evia_journal").joinpath("scenario.schema.json").read_text(encoding="utf-8")
        )
        self.assertEqual(
            installed,
    ...
    def test_cli_matches_library(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "evia_journal", "analyze", str(EXAMPLE), str(output)],
                capture_output=True,
            )
    ...
    def test_invalid_cli_preserves_last_report(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "evia_journal", "analyze", str(source), str(output)],
                capture_output=True,
            )
    ...
    def test_cli_refuses_input_overwrite(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "evia_journal", "analyze", str(source), str(source)],
                capture_output=True,
            )
            self.assertEqual(result.returncode, 1)
...
def parity_case(filename):
    def check(self):
        ...
        for key in ("records", "rootHash", "cutoffMs", "nowMs", "retentionMs"):
